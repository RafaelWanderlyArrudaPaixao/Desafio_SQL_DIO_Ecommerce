...
select PName, Category, sum(Quantity) as Menor_qtd_estoque
	from product p
    inner join productStock ps
		on p.IdProduct = ps.IdProduct
	group by p.IdProduct
	order by Menor_qtd_estoque;
...
select Category, count(IdOrder) as Qtd_Pedido_por_Categoria
	...
	group by Category
    order by Qtd_Pedido_por_Categoria desc;
...
select TypePayment, count(*) as Qtd_formas_pagamentos
    from payments p
    group by TypePayment
    order by Qtd_formas_pagamentos desc;
...
select PName, Category, round(sum(Category * Price), 2) as Receita_total_por_produto
    ...
    inner join productOrder po
		on p.IdProduct = po.IdProduct
	group by p.IdProduct
    order by Receita_total_por_produto desc;
